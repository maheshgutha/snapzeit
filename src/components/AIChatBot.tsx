...
export function AIChatBot() {
  ...
  // Fetch photographers only when the chat is opened to avoid background RPC calls
  ...
  const fetchPhotographers = async () => {
    try {
      const res = await apiClient.rpc('get_public_photographers');
      // supabase.rpc can return different shapes depending on client; handle safely
      const data = (res as any)?.data ?? (res as any)?.body ?? res;

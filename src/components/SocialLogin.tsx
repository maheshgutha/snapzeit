...
export const SocialLogin = ({ onSuccess, className = '', isPhotographer = false }: SocialLoginProps) => {
  ...
  const handleGoogleLogin = async () => {
    setLoading(true);
    try {
      const redirectUrl = isPhotographer 
        ? `${window.location.origin}/photographer/onboarding`
        : `${window.location.origin}/photographers`;
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <Button
        ...
        disabled={loading}
        className="w-full h-12 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 font-semibold"
      >
        {loading ? (

  ...
  const loadAdminData = async () => {
    try {
      ...
      const [usersCount, photographersCount, bookingsData, verificationsCount, disputesCount] = await Promise.all([
        supabase.from('user_roles').select('*', { count: 'exact' }).eq('role', 'user'),
        supabase.from('user_roles').select('*', { count: 'exact' }).eq('role', 'photographer'),
        supabase.from('bookings').select('total_amount'),
        supabase.from('photographer_verifications').select('*', { count: 'exact' }).eq('status', 'pending'),
